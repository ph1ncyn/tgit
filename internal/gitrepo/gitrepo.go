...
// sanitizeCR убирает из текста управляющие ASCII-байты перед показом в
// терминале — не только \r (см. историю этой функции), но и вообще любые
// C0-control байты, включая ESC (0x1B). Diff содержимого файла — это байты
// САМОГО файла как есть (git diff их не экранирует): CRLF-окончания строк,
// бинарный контент по ошибке распознанный как текстовый, да и просто файл,
// который сам зачем-то содержит control-байты. Если пропустить их в
// терминал без изменений, любой из них может быть исполнен КАК КОМАНДА
// терминала — голый \r переносит курсор в начало строки, а ESC способен
// запустить произвольную ANSI-последовательность (передвинуть курсор,
// стереть экран, что угодно), а не просто испортить одну строку. \r\n
// схлопывается в \n, остальные control-байты (кроме \t и \n, которые
// безопасны и нужны для форматирования) вырезаются целиком.
func sanitizeCR(s string) string {
	s = strings.ReplaceAll(s, "\r\n", "\n")
	s = strings.ReplaceAll(s, "\r", "\n")
	if !strings.ContainsFunc(s, isUnsafeControlByte) {
		return s
	}
	var b strings.Builder
	b.Grow(len(s))
	for _, r := range s {
		if !isUnsafeControlByte(r) {
			b.WriteRune(r)
		}
	}
	return b.String()
...
func isUnsafeControlByte(r rune) bool {
	if r == '\n' || r == '\t' {
		return false
	}
